...
export default function GlicoseCadastro() {
  ...
  // máscaras
  const fmtCpf  = t => t.replace(/\D/g,'').slice(0,11)
    .replace(/(\d{3})(\d{3})(\d{3})(\d{2})?/,(_,a,b,c,d)=>d?`${a}.${b}.${c}-${d}`:c?`${a}.${b}.${c}`:b?`${a}.${b}`:a);
  const fmtData = t => t.replace(/\D/g,'').slice(0,8)
    .replace(/(\d{2})(\d{2})(\d{4})?/,(_,d,m,y)=>y?`${d}/${m}/${y}`:m?`${d}/${m}`:d);
  const fmtTel  = t => t.replace(/\D/g,'').slice(0,11)
  ...
  const openSexoSelector = () => {
    ActionSheetIOS.showActionSheetWithOptions(
      { title:'Selecione o sexo', options:['Masculino','Feminino','Cancelar'], cancelButtonIndex:2 },
      i=>{ if(i===0) setSexo('M'); if(i===1) setSexo('F'); }
    );
  };
  const openUfSelector = () => {
    const opts=[...UF_LIST,'Cancelar'];
    ActionSheetIOS.showActionSheetWithOptions(
      { title:'Selecione o estado', options:opts, cancelButtonIndex:opts.length-1 },
      i=>{ if(i<UF_LIST.length) setRgUf(UF_LIST[i]); }
    );
  };
  const openLocSelector = () => {
    ...
    ActionSheetIOS.showActionSheetWithOptions(
      ...
      i=>{
        if(i<labels.length){ setLocSelecionado(String(locais[i].id)); setNovoLocal(''); }
        else if(i===labels.length){ setLocSelecionado(OUTRO_VALUE); setNovoLocal(''); }
      }
    );
  };
  ...
  const handleBuscarCpf = async () => {
    ...
    if(digits.length!==11){ Alert.alert('Atenção','CPF precisa ter 11 dígitos.'); return; }
    const net=await NetInfo.fetch();
    if(!net.isConnected){ Alert.alert('Offline','Sem internet.'); setHasSearched(true); return; }
    setSearching(true);
    try {
      ...
      err.response?.status===404
        ? Alert.alert('Não encontrado','CPF não cadastrado.')
        : Alert.alert('Erro','Falha ao consultar CPF.');
    } finally {
      setSearching(false);
      setHasSearched(true);
    }
  ...
  const handleSalvar = async () => {
    ...
    if (!cpfSearch || !nome.trim() || dataN.length!==10 ||
        !sexo || !rg.trim() || !rgUf || !telefone.trim() ||
    ...
    const payloadPac = {
      cpf: cpfSearch.replace(/\D/g,''), nome: nome.trim(),
      dataNascimento:`${y}-${m}-${d}`, sexo,
      rg: rg.replace(/\D/g,''), rgUf,
    ...
    try {
      ...
      await api.post('/resulsexame/novo',{
        idAgenda:null,idProced:ID_PROCED_GLICOSE,
        idPaciente:idPac,idLocColet:idLoc,
        resultado:null,observacao,
        dataCole:new Date().toISOString().slice(0,19).replace('T',' ')
      });

      Alert.alert(
  'Sucesso',
  'Paciente e exame de glicose cadastrados!',
  [{
    text: 'OK',
    onPress: () => nav.goBack()
  }]
                ...
                <View style={styles.formContainer}>
                  ...
                  {Platform.OS==='ios' ? (
                    ...
                    <View style={localStyles.pickerWrapper}>
                      <Picker selectedValue={rgUf} onValueChange={setRgUf}>
                        ...
                        {UF_LIST.map(u=><Picker.Item key={u} label={u} value={u}/>)}
                      </Picker>
                    </View>
                  )}
                  ...
                  {Platform.OS==='ios' ? (
                    <TouchableOpacity style={localStyles.select} onPress={openLocSelector}>
                      <Text>
                        {!locSelecionado?'Selecione':
                         locSelecionado===OUTRO_VALUE?'Outro...':
                    ...
                    <View style={localStyles.pickerWrapper}>
                      <Picker
                        ...
                        onValueChange={v=>{ setLocSelecionado(v); if(v!==OUTRO_VALUE) setNovoLocal(''); }}
                      >
                        <Picker.Item label="Selecione" value=""/>
                        {locais.map(l=><Picker.Item key={l.id} label={l.DESCRICAO} value={String(l.id)}/> )}
